...
def discretemap(colormap, hexclrs):
    ...
    cdict = {}
    for k_i, key in enumerate(('red', 'green', 'blue')):
        cdict[key] = [(i/(len(clrs)-2.), clrs[i, k_i], clrs[i+1, k_i])
...
class TOLcmaps:
    ...
    def __rainbow_discrete(self, lut=None):
        ...
        indexes = [
            ...
            [1, 3, 4, 6, 7, 8, 9, 11, 13, 14, 15, 16, 17, 19, 21, 23, 25, 26,
             27],
            [1, 3, 4, 6, 7, 8, 9, 10, 12, 13, 14, 15, 16, 17, 19, 21, 23, 25,
             26, 27],
            [1, 3, 4, 6, 7, 8, 9, 10, 12, 13, 14, 15, 16, 17, 18, 20, 22, 24,
             25, 26, 27],
            [1, 3, 4, 6, 7, 8, 9, 10, 12, 13, 14, 15, 16, 17, 18, 20, 22, 24,
             25, 26, 27, 28],
...
def tol_cset(colorset=None):
    ...
    if colorset == 'bright':
        ...
        the_cset = cset('#4477AA', '#EE6677', '#228833', '#CCBB44', '#66CCEE',
                        '#AA3377', '#BBBBBB', '#000000')

    elif colorset == 'high-contrast':
        cset = namedtuple('Hcset',
                          'blue yellow red black')
        the_cset = cset('#004488', '#DDAA33', '#BB5566', '#000000')

    elif colorset == 'vibrant':
        cset = namedtuple('Vcset',
                          'orange blue cyan magenta red teal grey black')
        the_cset = cset('#EE7733', '#0077BB', '#33BBEE', '#EE3377', '#CC3311',
                        '#009988', '#BBBBBB', '#000000')

    elif colorset == 'muted':
        cset = namedtuple('Mcset', 'rose indigo sand green cyan wine teal ' +
                          'olive purple pale_grey black')
        the_cset = cset('#CC6677', '#332288', '#DDCC77', '#117733', '#88CCEE',
                        '#882255', '#44AA99', '#999933', '#AA4499', '#DDDDDD',
                        '#000000')

    elif colorset == 'light':
        cset = namedtuple('Lcset', 'light_blue orange light_yellow pink ' +
...
#    plt.rc('image', cmap='rainbow_PuRd')
    ...
    for axs, scheme in zip(axes, schemes):
        cset = tol_cset(scheme)
        names = cset._fields
        colors = list(cset)
        for name, color in zip(names, colors):
            axs.scatter([], [], c=color, s=80, label=name)
        axs.set_axis_off()
        axs.legend(loc=2)
        axs.set_title(scheme)
    plt.show()
    ...
    for lut, axs in enumerate(axes, start=1):
        pos = list(axs.get_position().bounds)
        axs.set_axis_off()
